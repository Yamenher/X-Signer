...
public class KeyDate {
    ...
    public static String getKeyData(String keypath, String keystorepass, String type)
            throws Exception {
    ...
    private static KeyStore loadKeyStore(String keyStorePath, char[] password) throws Exception {
        if (Uri.parse(keyStorePath.toLowerCase())
                .getLastPathSegment()
                .substring(
                        (int)
                                (Uri.parse(keyStorePath.toLowerCase()).getLastPathSegment().length()
                                        - ".".concat("jks").length()),
                        (int) (Uri.parse(keyStorePath.toLowerCase()).getLastPathSegment().length()))
                .equals(".".concat("jks"))) {
            keyStore = KeyStore.getInstance("JKS", new JavaKeyStoreProvider());
        } else if (Uri.parse(keyStorePath.toLowerCase())
                .getLastPathSegment()
                .substring(
                        (int)
                                (Uri.parse(keyStorePath.toLowerCase()).getLastPathSegment().length()
